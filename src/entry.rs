...
///An occupied entry into a [MultiKeyMap].
    ...
    K: Hash + Eq,
{
    ...
    /// Removes the entry from the [MultiKeyMap], returning the key, and returning the
    ...
    /// Removes the entry from the [MultiKeyMap], and returns the
    ///  value if no other key references it.
...
///A vacant entry into a [MultiKeyMap].
