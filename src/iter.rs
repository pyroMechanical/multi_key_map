...
pub struct Iter<'a, K, V> where K: Hash + Eq{
    keys: std::collections::hash_map::Iter<'a, K, Index>,
    map: &'a HashMap<Index, (usize, V)>
}

impl<'a, K, V> Iter<'a, K, V> where K: Hash + Eq {
    pub fn new(map: &'a MultiKeyMap<K, V>) -> Self {
        let keys = map.keys.iter();
        Self{keys, map: &map.data}
    }
}

impl<'a, K, V> Iterator for Iter<'a, K, V> where K: Hash + Eq{
    type Item = (&'a K, &'a V);
